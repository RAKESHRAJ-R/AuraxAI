...
npm run test-tanglish  # Tanglish quality + machine-output leakage (150 checks) — from the
                       # 2026-09-21 client report. Asserts no tool-call/JSON/escape fragment
                       # and no broken markdown can reach a customer on ANY reply path, that
                       # good replies and the store's own FAQ pass through untouched, and that
                       # the invented Tamil seen in production is detected. Stubbed LLM.
...
Five of the highest-frequency conversational turns are handled entirely in code — no LLM call, no rate-limit exposure, no hallucination risk:
...
`npm run test-tanglish` — 150 checks against the real sanitiser, the real catalogue helpers and
